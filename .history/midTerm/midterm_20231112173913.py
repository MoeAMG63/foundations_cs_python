...
#&&&&&&&&&&&&&&&&&&&&Choice 8 &&&&&&&&&&&&&&&&&&&&&&
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
def loadTabs(file_path2):               #https://youtu.be/Uh2ebFW8OYM?si=YzpiCPBsRRc3ysF9
    try:
    ...
    try:
        if tab_index:                               # tab index is the user input scrape_url stores the url 
            ...
            if html_content.status_code == 200:
                ...
                print("Failed to scrape")           # this func webScrap is the switch tab choice i named it like that cause 
        else:
            tab = open_tabs[-1]
            scrape_url = tab["URl"]
